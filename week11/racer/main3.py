...
#Initialize the settings of the player's car and generate it in the specified position
    def __init__(self):
        ...
        self.image = pygame.image.load(r'C:\pp2\week11\racer\images\Player.png')
        self.rect = self.image.get_rect()
        self.rect.center = (W / 3, 520)
#Moves by player's commands on the x-axis
    def move(self):
        pressed = pygame.key.get_pressed()
        if pressed[pygame.K_w] and self.rect.top > 0: self.rect.move_ip(0, -10)
        if pressed[pygame.K_s] and self.rect.bottom < H: self.rect.move_ip(0, 10)
        if pressed[pygame.K_a] and self.rect.left > 0: self.rect.move_ip(-10, 0)
        if pressed[pygame.K_d] and self.rect.right < W: self.rect.move_ip(10, 0)
#Draws the player's car on the screen
    def draw(self, surface):
        surface.blit(self.image, self.rect)
...
while flRunning:
    ...
    score = font_small.render(f'PLAYER 1: {COIN2}', True, BLACK)
...
# Checks the collision of the player's car with a coin, if it becomes true, the player gets points in the form of a coin, also increases the speed of coins
    ...
    if pygame.sprite.spritecollideany(P2, coins):
        pygame.mixer.Sound(r'C:\pp2\week11\racer\sounds\coin.wav').play()
        C_COLLIDE = True
        C_SPEED += 0.2
        COIN2 += 1
    ...
    coin = font_small.render(f'PLAYER 2: {COIN1}', True, BLACK)
    sc.blit(coin, (270, 10))
