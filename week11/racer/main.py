...
#Moves by player's commands on the x-axis and y-axis
    def move(self):
...
##Initializes coin settings and generates them randomly on the screen
    def __init__(self):
        super().__init__()
        self.image = pygame.image.load(r'C:\pp2\week11\racer\images\Coin.png')
        self.rect = self.image.get_rect()
        self.rect.center = (random.randint(30, W - 30), random.randint(30, H - 30))
#Genrates a coin 
    def move(self):
        self.rect.center = (random.randint(30, W - 30), random.randint(30, H - 30))
...
while flRunning:
    ...
    for car in all_sprites:
        ...
        car.move()
#Checks the collision of the player's car with a coin, if it becomes true, the player gets points in the form of a coin, coin gets new positions
    if pygame.sprite.spritecollideany(P1, coins):
        pygame.mixer.Sound(r'C:\pp2\week11\racer\sounds\coin.wav').play()
        COIN += 1
        C1.move()
    
    coin = font_small.render(str(COIN), True, BLACK)
    sc.blit(coin, (370, 10))
